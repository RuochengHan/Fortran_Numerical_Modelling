...
program convect_2d
  ...
  namelist /inputs/ Nx, Ny, Ra, thr, kp, alpha, a_diff, a_adv, total_time, &
  ...
  do while (time < total_time)
    ...
    k = 1
    do while(res > thr)
      res = abs(Vcycle_2DPoisson(w, rhs, h, alpha)) / rms
    ...
    k = 1
    do while(res > thr)
      res = abs(Vcycle_2DPoisson(S, w, h, alpha)) / rms
      !print*, k, " 's iteration residue is: ", res
      if (res > 1e5) then
        stop "Diverge ! Please try smaller alpha"
      end if
    ...
    call renew(T, v_dT, vx, vy, tmp, h, kp, dt)
...
contains
	subroutine renew(T, v_dT, vx, vy, tmp, h, kp, dt)
	  use deriv
    ...
    integer, parameter                   :: dp = selected_real_kind(15, 307)
	  real(kind=dp), allocatable           :: ddT(:, :), v_dT(:, :), tmp(:, :), &
                                            T(:, :), vx(:, :), vy(:, :)
	  real(kind=dp), intent(in)            :: dt, h, kp
	  integer                              :: i, j, nx, ny
		nx = size(T, 1)
		ny = size(T, 2)
		allocate(ddT(nx, ny))
